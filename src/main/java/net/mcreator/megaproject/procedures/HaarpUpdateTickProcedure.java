...
for (int index0 = 0; index0<(int)(128); index0++) {if (((world.getBlockState(new BlockPos((int)x,(int)(y-        (iteration)
),(int)z))).getBlock()== HeavyframeItem.block.getDefaultState().getBlock())) {        heavyframes =(double)(        (heavyframes)
+1);
...
<128)) {{
	...
	BlockState _bs = HaarpinactiveItem.block.getDefaultState();
...
==(false)))) {    world.getWorld().getGameRules().get(GameRules.DO_DAYLIGHT_CYCLE).set((false),world.getWorld().getServer());
if(world instanceof World) world.getWorld().setDayTime((int) 1);        counted =(boolean)(true);
}
