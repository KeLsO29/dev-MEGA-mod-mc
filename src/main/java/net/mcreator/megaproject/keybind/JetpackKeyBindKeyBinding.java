...
import net.mcreator.megaproject.procedures.JetpackKeyBindOnKeyReleasedProcedure;
import net.mcreator.megaproject.procedures.JetpackKeyBindOnKeyPressedProcedure;
...
public class JetpackKeyBindKeyBinding extends MegaProjectModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	private long lastpress = 0;
	public JetpackKeyBindKeyBinding(MegaProjectModElements instance) {
		super(instance, 230);
	...
	public void initElements() {
		keys = new KeyBinding("key.mcreator.jetpack_key_bind", GLFW.GLFW_KEY_SPACE, "key.categories.movement");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					...
					MegaProjectMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
